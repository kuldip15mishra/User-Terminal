...
var Users = {
    ...
    getUserById: function (id, callback) {
        return db.query("select FirstName, LastName, Email, Mobile, City, State, Pincode, CardNumber, Age, Gender, MaritalStatus, Profession, UserID, isOTP from user where CardNumber=?", [id], callback);
    },
    ...
    updateUser: function (id, Users, callback) {
        return db.query("update user set FirstName=?,LastName=?,Email=?,Mobile=?,City=?,State=?,Pincode=?,Age=?,Gender=?,MaritalStatus=?,Profession=?,isOTP=? where CardNumber=?", [
            Users.FirstName, Users.LastName, Users.Email, Users.Mobile, Users.City,
            Users.State, Users.Pincode,
            Users.Age, Users.Gender, Users.MaritalStatus, Users.Profession, Users.isOTP, id], (err, result) => {
                if(result) {
                    if(result.affectedRows) {
                        return db.query("update card set FirstName=?,LastName=? where UserID=? and CardNumber=?", [
                            Users.FirstName, Users.LastName, Users.UserID, id
                        ], callback)
                    }
                } else {
    ...
    signupUser: function (Users, callback) {
        return db.query("update user set Password=?,Mobile=?,isOTP=? where CardNumber=?", [
            md5(Users.Password), Users.Mobile, Users.isOTP, Users.CardNumber], callback);
    },
