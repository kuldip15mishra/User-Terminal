...
var reportChangeDevice = {
    ...
    addReportChangeDevice: function (ReportChangeDevice, callback) {
        return db.query("Insert into reportchangedevice values(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [
            ...
            ReportChangeDevice.New_AdPanelID,
            ReportChangeDevice.Report_Date,ReportChangeDevice.Status],
            (error, result) => {
                if (result ) {
                    return db.query(`select id from reportchangedevice where UserID=? &&  ReportID=?`,
                     [ReportChangeDevice.UserID,ReportChangeDevice.ReportID], callback)
                } else {
