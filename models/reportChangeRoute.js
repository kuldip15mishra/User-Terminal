...
var reportChangeRoute = {
    ...
    addReportChangeRoute: function (reportChangeRoute, callback) {
        return db.query("Insert into reportchangeroute values(NULL,?,?,?,?,?,?,?,?,?)", [
            ...
            reportChangeRoute.Report_Date,reportChangeRoute.Status,reportChangeRoute.New_UserID],
            (error, result) => {
                if (result ) {
                    return db.query(`select id from reportchangeroute where UserID=? &&  ReportID=?`,
                     [reportChangeRoute.UserID,reportChangeRoute.ReportID], callback)
                } else {
